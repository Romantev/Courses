...
imagearray = () => {
  ...
  for (let i = 1; i <= 100; i++) {
    if (i < 10) {
      returnArray.push(`${i - 1}: image_00${i}.jpg`);
    } else if (i < 100) {
      returnArray.push(`${i - 1}:image_0${i}.jpg`);
    } else {
      returnArray.push(`${i - 1}:image_${i}.jpg`);
    }
  }
...
loopMe2 = () => {
  ...
  } else if (inputNum2 > 0 && inputNum2 % 2 != 0) {
    inputArray2.push("L");
    for (let i = 0; i < inputNum2; i++) {
